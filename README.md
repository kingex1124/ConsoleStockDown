...
- 呼叫 TWSE OpenAPI 取得上市公司每月營業收入彙總表
- 自動建立 SQLite `StockDaily` 資料表
- 自動建立 SQLite `InstitutionalTradeDaily` 資料表
- 自動建立 SQLite `MonthlyRevenueSummary` 資料表
...
- 月營收資料每日都可執行，但若同一個 `RevenueMonth` 已存在就會自動跳過，避免重複寫入
...
19. 呼叫 `https://openapi.twse.com.tw/v1/opendata/t187ap05_L`
20. 解析每筆上市公司月營收資料，將 `資料年月` 轉成 `RevenueMonth`、`出表日期` 轉成 `ReportDate`
21. 若 `MonthlyRevenueSummary` 已存在相同 `RevenueMonth` 的資料則跳過，否則整批寫入新月份資料
22. 將執行結果寫入 Console 與日誌檔
...
```json
{
  "AppSettings": {
    ...
    "MonthlyRevenueApiUrl": "https://openapi.twse.com.tw/v1/opendata/t187ap05_L",
...
| `MonthlyRevenueApiUrl` | 上市公司每月營業收入彙總表 API 位址。系統會每日呼叫，但只在抓到尚未存在的新 `RevenueMonth` 時才寫入 `MonthlyRevenueSummary`。 |
...
- 月營收 API 的 `資料年月` 若為民國年月格式，例如 `11505`，會轉成 `yyyy-MM`
...
- 月營收資料會以 `RevenueMonth` 判斷是否已存在，若相同月份已寫入則略過，不重複新增
- `ChangeRate` 計算方式為：
...
- `Services/MonthlyRevenueSummaryService.cs`
  實作上市公司每月營業收入彙總資料抓取、月份正規化與「已存在月份就跳過」的寫入流程。
...
- `Repository/MonthlyRevenueSummaryRepository.cs`
  封裝 `MonthlyRevenueSummary` 的建表、月份存在判斷與整批新增邏輯。
...
- `Models/MonthlyRevenueSummary.cs`
  定義 `MonthlyRevenueSummary` 資料表欄位與對應模型。
...
- 月營收資料放在獨立的 `MonthlyRevenueSummary` 資料表，並固定在主流程最後執行，不影響日線與法人資料同步。
...
- 同一個 `RevenueMonth` 的月營收資料只會新增一次；後續每日執行若 API 仍回傳相同月份，系統會記錄並跳過。
